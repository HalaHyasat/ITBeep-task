...
<div class="row d-flex justify-content-center">
        <div class="col-lg-5 form_container">
          <form id="services_form" onsubmit="event.preventDefault()">
              {{csrf_field()}}
              <p style="font-size: 100px; text-align:center; margin-bottom: 1rem">LOGO</p>
              ...
              <div class="modal-content">
                ...
                <div class="modal-body d-flex justify-content-between">
                    @foreach($interests as $interest)
                      <button class="btn" name="interests" style="border: width:32%" onclick="setInterests('{{$interest->name}}',event)" >
                        {{$interest->name}}
                      </button>
                    @endforeach
                </div>
...
@section("scripts")
    ...
    function setServices(name,event){
      if(event.target.style.border=="none"){
        event.target.style.border="solid 3px gray";
        services.push(name);
      }
      else{
